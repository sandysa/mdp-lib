...
void make_new_problem(State ssss, FILE *problem_file);
...
int random_walk_limit = 10;
int gnum_ractions;
float g_path_prob = 1;
float g_path_prob_limit = 0.0;
...

{
  ...
  action_table_type action_table[NUM_ACTIONS];
  ...
  /* syoon random walk generation */
  {
    FILE *at_file = fopen("action_table", "r");
    char *tchar = malloc(10);

    fscanf(at_file, "%d\n", &gnum_ractions);
    

    for(i=0; i < gnum_ractions;i++) {
      /*fscanf(at_file, "%s %d ", &(action_table[i].name), &(action_table[i].num_prob_effects));*/
      fscanf(at_file, "%s ", &(action_table[i].name));
      fscanf(at_file, "%d ", &(action_table[i].num_prob_effects));
      action_table[i].probs = (int *)malloc(sizeof(int) * action_table[i].num_prob_effects);
      action_table[i].prob_name = (char **)malloc(sizeof(char *) * action_table[i].num_prob_effects);
      for(j=0; j< action_table[i].num_prob_effects; j++) {
	action_table[i].prob_name[j] = (char *)malloc(sizeof(char) * (LEN_ACTION_NAME  + 10));
	fscanf(at_file, "%s ", action_table[i].prob_name[j]) ;
	fscanf(at_file, "%d ", &(action_table[i].probs[j]));
      }
      fscanf(at_file, "\n");
    }

    for(i=0; i<gnum_ractions;i++) {
      printf("%d %s %s\n",i, action_table[i].prob_name[0], action_table[i].prob_name[1]);
    }
  ...
  /* now try random walk */
  {
    int random_action_num, random_done, random_done_b;
    State temp_state, tmp_state;
    int random_limit;
    FILE *b_file_det = fopen("b-problem", "a");
    FILE *b_file_prob = fopen("b-problem_reset", "a");

    srand( (unsigned int)time( NULL ) );
    make_state(&temp_state, gnum_ft_conn);
    make_state(&tmp_state, gnum_ft_conn);
    source_to_dest(&temp_state, &ginitial_state);

    random_limit = 0;

    while( (random_limit < random_walk_limit) && (g_path_prob > g_path_prob_limit) ) {

      printf("random_walk %d \n", random_limit);
      
      get_A(&temp_state);

      if(gnum_A == 1) {
	/*print_goal_state(temp_state);*/
	make_new_problem(temp_state, b_file_det);
	make_new_problem(temp_state, b_file_prob);	
	printf("exiting only bogus action available \n");
	exit(100);
      }
	...
	random_done = 0;
	while(!random_done) {
	  random_action_num = rand() % gnum_A;
	  if( strcmp(gop_conn[gA[random_action_num]].action->name, "BOGUS") )
	    random_done = 1;
	}
    

	print_op_name(gA[random_action_num]);
	printf("random action num %d %d \n", random_action_num, gnum_A);
	...
	*/
	...
	random_action_num = select_action(gA[random_action_num], action_table);


	printf("SELECTED :::**"); print_op_name(random_action_num); printf("\n");

      

	if(strcmp(gop_conn[random_action_num].action->name, "BOGUS"))
	  random_done_b = 1;

	if(random_done_b) {
	  result_to_dest(&tmp_state, &temp_state, random_action_num);
	  source_to_dest(&temp_state, &tmp_state);
	}
	
      }

      random_limit++;
      printf("%d %d %f %f\n", random_limit, random_walk_limit, g_path_prob,
	     g_path_prob_limit);
    }
...

{
  ...
  while ( --argc && ++argv ) {
	...
	case 'd':
	  ...
	  break;
	case 'l':
	  sscanf( *argv, "%d", &random_walk_limit );
	  break;
	case 't':
...
int select_action(int random_action_num, action_table_type *action_table) 
{
  int i,j;
  char* cur_action_name = gop_conn[random_action_num].action->name;
  int found, selection_c = rand() % 100;
  int selected_action, selected_prob;
  ...
  for(i=0; i<gnum_ractions;i++) {
    found = 0;
    for(j=0; j< action_table[i].num_prob_effects; j++)
      if(!strcmp(cur_action_name, action_table[i].prob_name[j]))
	found = 1;
    if(found)
      break;
  }
  ...
  for(j=0; j< action_table[i].num_prob_effects; j++) {
    if( selection_c < action_table[i].probs[j] )
      break;
    else
      selection_c -= action_table[i].probs[j];
  }
  ...
  for(i=0; i< gnum_A; i++) {
    if(!strcmp(gop_conn[gA[i]].action->name,
	       action_table[selected_action].prob_name[selected_prob])) {
      for(j=0; j< gop_conn[gA[i]].action->num_name_vars; j++)
	if( gop_conn[gA[i]].action->name_inst_table[j] !=
	    gop_conn[random_action_num].action->name_inst_table[j])
	  break;
      if(j == gop_conn[gA[i]].action->num_name_vars)
	break;
    }
  }

  g_path_prob = g_path_prob * ((float)action_table[selected_action].probs[selected_prob] / (float)100);
  
  return(gA[i]);
  
}

void print_goal_state(State ssss) 
{

  int i,j;

  for(i=0; i< ssss.num_F; i++) {
    for(j=0; j< ginitial_state.num_F; j++) 
      if( ssss.F[i] == ginitial_state.F[j] )
	break;

    if( j != ginitial_state.num_F )
      continue;

    print_ft_name(ssss.F[i]); printf("\n");
  }
...
void make_new_problem(State ssss, FILE *problem_file) 
{
  ...
  int i,j;
  int object_num, fact_num;
  Fact *f;
  
  fprintf(problem_file, " (:goal (and \n");


  for(fact_num = 0; fact_num < ssss.num_F; fact_num++) {
    for(j=0; j< ginitial_state.num_F; j++) 
      if( ssss.F[fact_num] == ginitial_state.F[j] )
	break;

    if( j != ginitial_state.num_F )
      continue;

    f = &grelevant_facts[ssss.F[fact_num]];


    if( f-> predicate > (gnum_predicates/2) ) {
      printf("%d (%s ", f->predicate, gpredicates[f->predicate]);
      continue;
    }
    

    fprintf(problem_file, "(%s ", gpredicates[f->predicate]);

    for ( j=0; j<garity[f->predicate]; j++ ) {
      if ( f->args[j] >= 0 ) {
	printf("%s ", gconstants[(f->args)[j]]);
	fprintf(problem_file, "%s ", gconstants[(f->args)[j]]);
      } else {
	printf("x%d ", DECODE_VAR( f->args[j] ));
	fprintf(problem_file, "x%d ", DECODE_VAR( f->args[j] ));
      }
