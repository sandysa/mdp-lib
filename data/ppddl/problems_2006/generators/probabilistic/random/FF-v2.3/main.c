...
{
  FILE *rfile = fopen(routputfile, "a");
  ...
  fprintf(rfile, "planlength %d plantime %7.2f searchstates %d maxdepth %d\n", gnum_plan_ops,
	  gtempl_time + greach_time + grelev_time + gconn_time + gsearch_time,
...

{
  ...
  while ( --argc && ++argv ) {
	...
	case 'd':
	  ...
	  break;
	case 'r':
	  strncpy( routputfile, *argv, MAX_LENGTH );
	  break;
	default:
