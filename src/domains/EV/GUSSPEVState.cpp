...
GUSSPEVState::GUSSPEVState(mlcore::Problem* problem, int soc, int t, std::vector<std::pair<int, double>> goalPos)
{
    soc_ = soc;
    timestep_ = t;
    goalPos_ = goalPos;
...
int GUSSPEVState::hashValue() const
{
   int hashVal = soc_ + 31* timestep_;
     for(int i = 0; i < goalPos_.size(); ++i){
        std::pair<int,double> pos = goalPos_[i];
        hashVal += 31 * pos.first ;
    }
    return hashVal;
